      ...
      try {
        const [dataResponse, textResponse, solutionsResponse] =
          await Promise.all([
            ...
            axios.get(
              `http://localhost:3002/api/search-text-data?page=${page}&limit=${limit}&query=${encodeURIComponent(
                query
              )}`
            ), // 쿼리 인코딩
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3002/api/search-text-data?page=1&limit=${limit}&query=${encodeURIComponent(
          query
        )}` // 쿼리 인코딩
      );
